...
{
    SDL_SetRenderDrawColor(g_renderer, 255, 0, 0, 255);//thiết lập giá trị màu
    SDL_RenderClear(g_renderer);//Xóa toàn bộ nội dung của framebuffer với giá trị màu đã được thiết lập
    fruitDraw(&g->f); /* vẽ mồi */
    snakeDraw(&g->s); /* vẽ rắn */
    SDL_RenderPresent(g_renderer);//Tiến hành sao chép toàn bộ nội dung của vùng nhớ đệm dựng hình lên cửa sổ
}
